...
source_diam <- "cockroach_diameter_data.csv"
...
dat_long <- dat_raw_main %>% 
  pivot_longer(-frame) %>% 
  na.omit() %>% 
  left_join(dat_diam, by = "name")

# create categories from data
sep_id <- c("roach_id", "roach_antenna","side")

dat_sep_1 <- dat_long %>% separate(name, c("name", "distance"), "_") 
dat_sep <- dat_sep_1 %>% 
  extract(name, sep_id,
...
dat_set <- dat_sep %>% full_join(check_dat, by = exp_id)
...
dat_result <- dat_set %>% 
  group_by(roach_id,roach_antenna,side,distance) %>% 
  mutate(num_weights = order(frame_total,frame), ascending = (frame <= floor(frame_total/2)))
...
norm_1 <- dat_norm %>% filter(frame <= floor(frame_total/2)) %>% # ascending
  ggplot(aes(weight,value, color = distance)) +
  geom_point() +
  geom_line() +
...
norm_ex <- dat_norm %>% filter(frame <= floor(frame_total/2), distance == "1") %>% # ascending
  ggplot(aes(weight,value, color = max_prop, fill = roach_id)) +
  geom_point() +
  geom_line() +
...
dat_hy <- dat_norm %>% 
  mutate(ascending = ifelse(frame == floor(frame_total/2), FALSE, ascending)) %>%
  bind_rows(hy_comp)

# 4x7
norm_hy <- dat_hy %>% group_by(ascending) %>%
  filter(roach_id == 10) %>%
  ggplot(aes(weight,value, color = distance, shape = ascending)) +
  geom_point() +
  geom_line() +
...
# fail graph
{# for(i in 1:20){
#   y1 <- min(dat_norm %>% mean(value[dat_norm$weight==weight[i]] & dat_norm$roach_id ==))
# }
# 
# norm_b <- dat_norm %>%
#   ggplot(aes(weight,y1 = min(value[dat_norm$weight==weight]) ,y2 = max(value[dat_norm$weight==weight]), color = distance, shape = ascending, fill = 'red')) +
#   geom_line(aes(y=y1)) +
#   geom_line(aes(y=y2)) +
#   facet_grid(roach_id~side) +
#   sca_col
...
for(i in 1:length(unique(tes$roach_id))){
  ...
  exp <- tes %>% filter(roach_id == v) %>% filter((frame <= floor(frame_total/2)))
  x <- exp$frame
  y <- exp$value
...
# area_1_d <- dat_norm[dat_norm$side == 'd',] %>% filter((frame <= floor(frame_total/2))) %>% sm_auc_all(subjects = 'roach_id', conditions = c('distance'), x = 'frame', values = 'value')
# area_2_d <- dat_norm[dat_norm$side == 'd',] %>% filter((frame >= floor(frame_total/2))) %>% sm_auc_all(subjects = 'roach_id', groups = c('distance'), x = 'frame', values = 'value')
# area_1_v <- dat_norm[dat_norm$side == 'v',] %>% filter((frame <= floor(frame_total/2))) %>% sm_auc_all(subjects = 'roach_id', groups = c('distance'), x = 'frame', values = 'value')
# area_2_v <- dat_norm[dat_norm$side == 'v',] %>% filter((frame >= floor(frame_total/2))) %>% sm_auc_all(subjects = 'roach_id', groups = c('distance'), x = 'frame', values = 'value')

# hister_d <- full_join(area_1_d,area_2_d, by = c('roach_id','distance')) %>% mutate(AUC_diff = AUC_value.y-AUC_value.x)
...
# tes %>% ungroup() %>% sm_auc_all(subjects = 'roach_id', conditions = 'distance', x = 'frame', values = 'value')
  }

# summary
{res1 <- dat_norm %>% group_by(roach_id,  roach_antenna, side, distance) %>% summarize(angle_max = max(value), weight_max = max(weight))
res2 <- dat_norm %>% filter(frame > floor(frame_total/2)) %>% group_by(roach_id,  roach_antenna, side, distance) %>% summarize(angle_min = min(value))

res <- full_join(res1,res2,by = c('roach_id',  'roach_antenna', 'side', 'distance')) %>%
  mutate(return_rate = 1-angle_min/angle_max)

res_d <- res %>% ungroup() %>% group_by(distance) %>% summarize(return_rate = mean(return_rate))
res_w <- res %>% ungroup() %>% group_by(weight_max, distance) %>% summarize(return_rate = mean(return_rate)) %>% arrange(desc(-weight_max))}

# grouped data
{res_w %>% ggplot(aes(weight_max,return_rate)) +
  geom_point() + theme_bw() 

res_w_1 <- res %>% ungroup() %>% group_by(weight_max) %>% mutate(return_rate = mean(return_rate)) %>% arrange(desc(-weight_max))
res_w_2 <- res %>% ungroup() %>% group_by(weight_max, side) %>% mutate(return_rate = mean(return_rate)) %>% arrange(desc(-weight_max))
...
{res_a <- res %>% ungroup() %>% group_by(angle_max) %>% mutate(return_rate = mean(return_rate)) %>% arrange(desc(-angle_max))
plot(res_a)
